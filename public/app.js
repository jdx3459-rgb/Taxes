...
function renderControls() {
  ...
  if (!hand || hand.phase === 'complete') { const canStart = state.hostId === socket.id && state.seats.filter(Boolean).length >= 2; container.innerHTML = canStart ? '<button class="start" id="start">开始下一手</button>' : `<p class="spectator-note">${state.hostId === socket.id ? '还需要一位玩家入座' : '等待桌主开始下一手'}</p>`; $('#start')?.addEventListener('click', () => socket.emit('start-hand')); return; }
...
function render() {
  ...
  $('#pot').innerHTML = `底池 <b>${hand?.pot?.toLocaleString() || 0}</b>`; $('#phase').textContent = phaseLabel[hand?.phase] || '等待下一手'; $('#message').textContent = hand?.message || '至少两位玩家入座后可以开始。'; $('#watchers').textContent = state.watcherCount;
