...
function publicState(viewerId) {
  ...
  return {
    seats: game.seats.map((player, seat) => player && ({
      id: player.id, name: player.name, chips: player.chips, seat, inHand: player.inHand,
    ...
    you: game.seats.find(player => player?.id === viewerId) ? { seated: true, seat: game.seats.findIndex(p => p?.id === viewerId) } : { seated: false }
  };
}
...
function finishHand(message) {
  ...
  playersInHand().forEach(player => { player.inHand = false; player.folded = false; player.allIn = false; player.streetBet = 0; player.totalBet = 0; });
  broadcast();
}
...
function startHand() {
  const ready = game.seats.filter(player => player && player.chips > 0);
  if (ready.length < 2) return;
  game.dealer = nextSeat(game.dealer, player => player.chips > 0);
  const deck = makeDeck();
  game.seats.forEach(player => { if (player) Object.assign(player, { inHand: player.chips > 0, folded: false, allIn: false, cards: player.chips > 0 ? [deck.pop(), deck.pop()] : [], streetBet: 0, totalBet: 0, acted: false }); });
...
io.on('connection', socket => {
  socket.on('enter', ({ password, name, mode }) => {
    ...
    if (mode === 'player') {
      ...
      else { game.seats[seat] = { id: socket.id, name: safeName, chips: STARTING_STACK, inHand: false, folded: false, allIn: false, cards: [], streetBet: 0, totalBet: 0, acted: false }; if (!game.hostId) game.hostId = socket.id; }
    } else game.watchers.set(socket.id, safeName);
    socket.emit('entered'); broadcast();
  });
  socket.on('start-hand', () => {
    if (socket.id !== game.hostId) return;
    if (game.hand && game.hand.phase !== 'complete') return;
    startHand();
  });
