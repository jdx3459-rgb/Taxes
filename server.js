...
function publicState(viewerId) {
  ...
  return {
    ...
    isAdmin: game.admins.has(viewerId),
...
function clearPlayerInfo() {
  ...
  const roomMembers = [...(io.sockets.adapter.rooms.get(ROOM) || [])];
  for (const socketId of roomMembers) {
    if (game.admins.has(socketId)) continue;
    const member = io.sockets.sockets.get(socketId);
    member?.leave(ROOM);
    member?.emit('room-cleared');
  }
  broadcast();
}
...
io.on('connection', socket => {
  socket.on('enter', ({ password, name, mode }) => {
    ...
    if (mode === 'admin') {
      if (!passwordsMatch(password, ADMIN_PASSWORD)) return socket.emit('entry-error', ADMIN_PASSWORD ? '管理员密码不正确' : '服务器尚未配置 ADMIN_PASSWORD');
      socket.join(ROOM);
      game.admins.set(socket.id, safeName);
      socket.emit('entered');
      return broadcast();
    }
    if (!passwordsMatch(password, ROOM_PASSWORD)) return socket.emit('entry-error', '房间密码不正确');
    socket.join(ROOM);
    ...
    if (mode === 'player') {
      ...
      else { game.seats[seat] = { id: socket.id, name: safeName, chips: STARTING_STACK, ready: true, queued: handInProgress(), inHand: false, folded: false, allIn: false, cards: [], streetBet: 0, totalBet: 0, acted: false }; joinedAsPlayer = true; if (!game.hostId) game.hostId = socket.id; }
    } else game.watchers.set(socket.id, safeName);
    socket.emit('entered');
    if (joinedAsPlayer) maybeStartHand();
    else broadcast();
  });
  ...
  socket.on('clear-player-info', () => {
    if (game.admins.has(socket.id)) clearPlayerInfo();
  });
  socket.on('broadcast-message', ({ text }) => {
    const player = game.seats.find(player => player?.id === socket.id);
    if (!player || !QUICK_MESSAGES.includes(text)) return;
    const now = Date.now();
    if (player.lastAnnouncementAt && now - player.lastAnnouncementAt < 2500) return;
    player.lastAnnouncementAt = now;
    const id = crypto.randomUUID();
    game.announcements = [...game.announcements, { id, from: player.name, text }].slice(-3);
    broadcast();
    setTimeout(() => {
      game.announcements = game.announcements.filter(message => message.id !== id);
      broadcast();
    }, 3000);
  });
